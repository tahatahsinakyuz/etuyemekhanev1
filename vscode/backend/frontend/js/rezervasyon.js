...
async function loadUserInfo() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Sunucudan beklenmeyen yanıt: ${response.status}`);
...
async function makeReservation(email, tarih, ogun, ucret) {
    try {
        const response = await fetch(`${API_URL}/api/rezervasyon-ekle`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
async function loadTransactionHistory() {
    ...
    if (!email) {
        transactionList.innerHTML = `<tr><td colspan="4">Kullanıcı giriş bilgisi eksik!</td></tr>`;
        return;
    }
    try {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    await loadTodayReservation();
    await loadReservationHistory();
    await loadTransactionHistory();
    // Eğer sadece admin sayfasında Tüm Rezervasyonlar gerekiyorsa şunu sadece oraya ekle:
