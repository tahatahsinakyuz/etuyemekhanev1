...
const dbConfig = {
    ...
    database: 'etuyemekhanev1', // Aynı olmalı!
    timezone: '+03:00'
};
...
async function adminMiddleware(req, res, next) {
    ...
    if (!email) return res.status(400).json({ success: false, message: "E-posta belirtilmedi!" });
    try {
...
// Rezervasyon Listeleme API'si (INJECTION KORUMALI!)
app.get("/api/rezervasyonlar", async (req, res) => {
    const { isim, tarih, ogun } = req.query;
    let query = "SELECT * FROM rezervasyonlar WHERE 1=1";
    let params = [];
    if (isim) { query += " AND isim_soyisim LIKE ?"; params.push(`%${isim}%`); }
    if (tarih) { query += " AND DATE(tarih) = ?"; params.push(tarih); }
    if (ogun) { query += " AND ogun = ?"; params.push(ogun); }
    try {
        const db = await connectToDatabase();
        const [results] = await db.execute(query, params);
        return res.json({ success: true, rezervasyonlar: results });
    } catch (error) {
        console.error("Rezervasyon listeleme hatası:", error);
        return res.status(500).json({ success: false, message: "Rezervasyonları getirirken bir hata oluştu." });
    }
});
...
/************************************************************************ 
 *               Dinamik Rezervasyon, Bakiye, Menü, Yorum               *
 ************************************************************************/

// Rezervasyon Ekleme (bakiye düşme, çift rezervasyon kontrolü)
app.post('/api/rezervasyon-ekle', async (req, res) => {
    ...
    try {
        ...
        // Bakiye işlemi geçmişine ekle (opsiyonel, şeman varsa kullan!)
        await db.execute(`
            INSERT INTO bakiye_islemleri (kullanici_id, tutar, islem_tipi, tarih, saat)
            VALUES (?, ?, ?, CURDATE(), CURTIME())
        `, [okulNo, -rezervasyonUcreti, 'Rezervasyon']);
...
// Rezervasyon geçmişi ve durum kontrolü
...
// Kullanıcının bakiye işlem geçmişi
...
// Haftalık menü (bugünden itibaren 7 gün)
...
// Bakiye yükle (hem bakiye güncelle hem işlem kaydı)
app.post('/api/bakiye-yukle', async (req, res) => {
    ...
    try {
        ...
        await db.execute(`
            INSERT INTO bakiye_islemleri (kullanici_id, tutar, islem_tipi, tarih, saat)
            VALUES (?, ?, ?, CURDATE(), CURTIME())
        `, [kullanici_id, miktar, 'Bakiye Yüklendi']);
...
app.post('/api/yorum-ekle', async (req, res) => {
    ...
    try {
        const db = await connectToDatabase();
        await db.execute(
            `INSERT INTO yorumlar (isim, eposta, yorum, tarih) VALUES (?, ?, ?, NOW())`,
            [isim, eposta, yorum]
        );
...
// Yorum beğeni/begenmeme işlemi - güvenli sütun seçimi!
app.post('/api/yorum-begeni', async (req, res) => {
    ...
    let column = null;
    if (action === "like") column = "begeni";
    else if (action === "dislike") column = "begenmeme";
    if (!column) return res.status(400).json({ success: false, message: "Geçersiz işlem!" });
